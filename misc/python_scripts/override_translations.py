...
def override_translation(input_file, path_to_coopbusiness):
    with open(input_file, 'r') as input_f:
        for line in csv.DictReader(input_f, delimiter=','):
            translation_file = get_translation_file(path_to_coopbusiness,
            ...
            if os.path.exists(os.path.abspath(os.path.join(
                        path_to_coopbusiness, 'modules', line['Module']))):
                sep = ''
            else:
                sep = '%s.' % line['Module']
...
def get_translation_file(path_to_coopbusiness, module, language):
    if os.path.exists(os.path.abspath(
            os.path.join(path_to_coopbusiness, 'modules', module))):
        # coog module
        return os.path.abspath(os.path.join(path_to_coopbusiness,
            'modules', module, 'locale', '%s.po' % language))
    elif os.path.exists(os.path.abspath(
            os.path.join(path_to_coopbusiness, 'modules',
        ...
        path = os.path.abspath(os.path.join, path_to_coopbusiness, 'modules',
                '%s_cog_translation' % module)
        os.makedirs(path)
