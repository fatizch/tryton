...
class RuleEngineRuntime(metaclass=PoolMeta):
    ...
    def _re_coverage_already_subscribed(cls, args):
        ...
        matches = []
        for c in contracts:
            covereds = [cc.party for cc in c.get_covered_elements_at_date(date)]
            if contract.subscriber in covereds:
                for o in c.options:
                    if (o.coverage == args['option'].coverage and
                            o.initial_start_date <= date and
                            (o.final_end_date or datetime.date.max) >= date and
                            o.status != 'void'):
                        matches.append(c.id)
                        continue
        matches.sort()
    ...
    def get_all_contracts(cls, party, at_date=None):
        ...
        return Contract.search([
                ('activation_history.start_date', '<=', at_date),
