...
# #Comment# #Create billing mode configuration
...
# Here the situation is as follow:
#
# There are two contracts:
#
#    - The first one is a manual contract, subscribed today, and for which
#      there is one generated invoice, which is due for today.
#    - The second one is a direct debit contract, subscribed a month ago
#      today, and for which there are two generated invoices, due for today as
#      well
...
# No dunnings yet, the lines are due for today
Dunning = Model.get('account.dunning')
Wizard('account.dunning.create').execute('create_')
assert_eq(len(Dunning.find([])), 0)

# #Comment# #Create dunnings at 19 days
config._context['client_defined_date'] = today_plus(19)

# Still nothing, we are one day early
Wizard('account.dunning.create').execute('create_')
assert_eq(len(Dunning.find([])), 0)

# #Comment# #Create dunnings at 20 days
config._context['client_defined_date'] = today_plus(20)

# Now we got one, for the "manual" contract (dunning first levels are only for
# manual contracts)
Wizard('account.dunning.create').execute('create_')
...
# #Comment# #Create dunnings at 39 days => Nothing changed
...
# #Comment# #Create dunnings at 40 days, manual_dunning goes draft next level
...
# #Comment# #Create dunnings at 60 days, new dunnings for direct debit (a
# little late but it happens)
config._context['client_defined_date'] = today_plus(60)
Wizard('account.dunning.create').execute('create_')
assert_eq(len(Dunning.find([])), 3)

# First dunning will still be manual_dunning, the two others are those of the
# direct debit contract. Ideally, they should be properly ordered by invoice
# date but they are not, their order is random :(
...
# is_contract_main is decided by id ordering (if everything else is the same),
...
# Nothing changed because is_contract_main is not True
assert_eq(direct_debit_dunning_2.state, 'draft')

assert_eq(direct_debit_contract.dunning_status, 'Formal Notice')

# #Comment# #Create dunnings at 70 days, manual_dunning goes draft next level
...
#
# This whole block should actually pass, but it does not because not_mandatory
# is not properly managed
#
# config._context['client_defined_date'] = today_plus(100)
# Wizard('account.dunning.create').execute('create_')
# manual_dunning.reload()
# direct_debit_dunning_1.reload()
# direct_debit_dunning_2.reload()
...
#
# # We ignore the Suspension state, since is is optionnal
# assert_eq(direct_debit_dunning_1.state, 'draft')
# assert_eq(direct_debit_dunning_1.level.name, 'Void Contract')
#
# # This one is not modified, because it is not is_contract_main
# assert_eq(direct_debit_dunning_2.state, 'draft')
# assert_eq(direct_debit_dunning_2.level.name, 'Formal Notice')
#
# Wizard('account.dunning.process', [manual_dunning, direct_debit_dunning_1,
#         direct_debit_dunning_2]).execute('process')
# assert_eq(manual_dunning.state, 'final')
# assert_eq(manual_dunning.last_process_date, today_plus(100))
# assert_eq(manual_dunning.contract.dunning_status, 'Void Contract')
# assert_eq(manual_dunning.contract.status, 'void')
# assert_eq(direct_debit_dunning_1.state, 'final')
# assert_eq(direct_debit_dunning_1.last_process_date, today_plus(100))
# assert_eq(direct_debit_dunning_1.contract.dunning_status, 'Void Contract')
# assert_eq(direct_debit_dunning_1.contract.status, 'void')
# assert_eq(direct_debit_dunning_2.state, 'draft')
# assert_eq(direct_debit_dunning_2.state, 'Formal Notice')

# #Comment# #Create dunnings at 85 days
...
# This one will not change anyway, since it is not is_contract_main
assert_eq(direct_debit_dunning_2.state, 'draft')
assert_eq(direct_debit_dunning_2.level.name, 'Formal Notice')
...
# The actual date is the previous state + 10
