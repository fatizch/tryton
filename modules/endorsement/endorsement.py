...
class Endorsement(QueueMixin, Workflow, Printable, model.CoogSQL,
    ...
    contracts_name = fields.Function(
        ...
        'get_contracts_name', searcher='search_contracts_name')
    subscribers_name = fields.Function(
        fields.Char('Subscribers Name'),
        'get_subscribers_name', searcher='search_subscribers_name')
    ...
    def get_contracts_name(self, name):
        return ', '.join(sorted([x.rec_name for x in self.contracts]))
    ...
    @classmethod
    def search_contracts_name(cls, name, clause):
        if clause[2] and ',' in clause[2]:
            ret_clause = ['OR']
            for subc in clause[2].split(','):
                if not subc:
                    continue
                subc = subc.strip()
                ret_clause += [('contract_endorsements.contract.rec_name',
                    clause[1], subc)]
        else:
            ret_clause = [('contract_endorsements.contract.rec_name',) +
                tuple(clause[1:])]
    ...
    @staticmethod
    def order_contracts_name(tables):
        contracts_name_order = tables.get('contracts_name_order', None)
        if contracts_name_order:
            return [contracts_name_order[None][0].order]
        ...
        table, _ = tables[None]
        join_table = contract_endorsement.join(endorsement,
            condition=contract_endorsement.endorsement == endorsement.id
            ).join(contract,
            condition=contract_endorsement.contract == contract.id)
        query = join_table.select(
            endorsement.id, contract.contract_number.as_('order'),
            order_by=[contract.contract_number.asc])

        tables['contracts_name_order'] = {
            None: (query, (query.id == table.id)),
            }
        return [query.order]

    @classmethod
    def search_subscribers_name(cls, name, clause):
        if clause[2] and ',' in clause[2]:
            ret_clause = ['OR']
            for subc in clause[2].split(','):
                if not subc:
                ...
                subc = subc.strip()
                ret_clause += [
                    ('contract_endorsements.contract.subscriber.rec_name',
                    clause[1], subc)]
        else:
            ret_clause = [
                ('contract_endorsements.contract.subscriber.rec_name',) +
    ...
    @staticmethod
    def order_subscribers_name(tables):
        parties_name_order = tables.get('parties_name_order', None)
        if parties_name_order:
            return [parties_name_order[None][0].order]
        ...
        query = join_table.select(
            endorsement.id, party.name.as_('order'),
            order_by=[party.name.asc])

        tables['parties_name_order'] = {
            None: (query, (query.id == table.id)),
            }
        return [query.order]
