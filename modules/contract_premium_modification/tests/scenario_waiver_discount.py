...
DiscountModification = Model.get('contract.premium_modification.discount')
...
# #Comment# Create a discount
discount = CommercialDiscount(name="Discount")
discount_rule = discount.rules.new()
discount_rule.rate = Decimal('0.5')
discount_rule.account_for_modification = product_account
discount_rule.invoice_line_period_behaviour = 'one_day_overlap'
discount_rule.taxes.append(tax_waiver)
discount_rule.coverages.append(OptionDescription(coverage2.id))
discount.save()
...
premium_1 = ContractPremium.create([{
            'option': contract.covered_elements[0].options[1].id,
            'start': contract_start_date,
            'amount': Decimal('50'),
            'frequency': 'monthly',
            'account': product_account.id,
            'rated_entity': (coverage2.__class__.__name__ + ','
                + str(coverage2.id)),
            }], config.context)
contract.save()
...
create_wizard = Wizard(
    'contract.premium_modification.create_discounts', [contract])
len(create_wizard.form.options)
# #Res# #1
create_wizard.form.options[0].coverage.code == 'test_coverage2'
# #Res# #True
create_wizard.form.start_date = contract_start_date
create_wizard.form.discount, = create_wizard.form.possible_discounts
...
all_invoices[1].invoice.state
# #Res# #'posted'
all([(x.invoice.total_amount, x.invoice.state) == (127.5, 'posted')
    for x in all_invoices[2:]])
# #Res# #True

# #Comment# #Test Set Waiver End Date Wizard
dms = DiscountModification.find([])[0]
end_date_wizard = Wizard('contract.premium_modification.set_end_date', [dms])
