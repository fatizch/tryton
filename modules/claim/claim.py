...
class Claim(export.ExportImportMixin, Printable, model.CoogView):
    ...
    def __register__(cls, module):
        ...
        # Migration from 2.6
        # Migration that allow the claim sequence
        # 'claim_sequence' that is auto generated by the associated Xml
        # to be editable inside coog client (noupdate=True)
        model_data_table = Pool().get('ir.model.data').__table__()
        cursor = Transaction().connection.cursor()
        cursor.execute(*model_data_table.update(
            [model_data_table.noupdate], [True],
            where=(
                (model_data_table.fs_id == 'claim_sequence') &
                (model_data_table.module == 'claim')
                )))
