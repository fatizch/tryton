...
def generate(output_doc_directory=None):
    logging.basicConfig()
    ...
    for module in os.listdir(modules_doc_files):
        ...
        try:
            with codecs.open(os.path.join(modules_doc_files, module_translated,
                        'doc', language, 'index.rst'), encoding='utf-8') as \
                    index, codecs.open(os.path.join(modules_doc_files,
                        module_translated, 'doc', language, 'summary.rst'),
                    encoding='utf-8') as summ:
