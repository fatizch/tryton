...
Création de la table et de ces paramètres d'entrée
..................................................
...
.. note:: Il est possible de copier coller les valeurs des dimensions depuis un tableur. Pour cela aller dans la définition d'une dimension. Créer une première valeur. Copier vos données depuis un tableur. Faites un coller (ctrl V ou clic droit coller) sur la cellule correspondant à la colonne à remplir et sur la première ligne. Vérifier que cette cellule n'est pas en édition avant de coller.
...
Les intervalles de la dimension Age sont formatés sous la forme [borne_inferieure - borne_superieure[. Les dimensions de type valeur correspondent n'ont pas de formatage spécifique et reprennent aux valeurs définies. Dans l'exemple, il faut retravailler la colonne Age de cette façon.
...
.. note:: Si la méthode *import par copier coller* est utilisée, l'ordre des colonnes à une importance. Par contre le titre de la colonne n'est pas nécessaire. Dans le cas d'un *import par fichier*, l'ordre des colonnes n'a pas d'importance mais pas contre le titre des colonnes doit correspondre au nom de la dimension. 
...
Cette méthode peut être utilisée dans tout les cas.
...
Depuis cet écran il faut:
    ...
    - appuyer sur 'Selection automatique': si le nom des colonnes du fichier CSV correspond aux dimensions de la table les champs à importer vont être automatiquement initialisés. 
...
Dans notre premier exemple, la règle va retourner le montant défini dans la table précédente. On a trois paramètres dans notre règle.  
    ...
    - le statut fumeur: le statut fumeur est une donnée complémentaire qui n'existe pas dans Coog et qui doit être ajoutée par configuration en enrichissant manuellement l'arbre de gauche. Pour cela cocher la case 'Afficher plus de données' et sélectionner le type 'Données Complémentaires'. Ajouter une donnée complémentaire. Pour cela, dans la colonne 'Données Complémentaires Externe' entrer fumeur. Cooq vous propose les données complémentaires déjà créées. Si le statut fumeur n'existe pas vous pouvez le créer en renseignant les champs de la façon suivante
